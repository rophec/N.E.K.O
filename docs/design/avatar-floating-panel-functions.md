...
Day 2-4 的台词目前集中在 `static/tutorial/yui-guide/director.js` 的 `AVATAR_FLOATING_GUIDE_ROUNDS` 中。文本 key 必须稳定，便于后续只替换文案和音频；如果后续多语言文案膨胀，再拆到类似 `static/tutorial/yui-guide/steps.js` 或新增 `static/avatar-floating-guide-steps.js`。
...
- `static/avatar-performance-stage.js`、`static/tutorial/avatar/yui-stage.js`：模型演出运行时和首页适配层。

...
| `pngtuber` | （通过 model_manager.html 管理） | pngtuber-core.js |
...
后端也通过 `TranslationService`（`utils/language_utils.py`）支持翻译：

- 首选：`googletrans` 库
- 回退：`translatepy` 库
- 最终回退：基于 LLM 的翻译

会话管理器（`main_logic/core.py`）仅通过 `_get_translation_service()` 访问器暴露该服务，本身并不在语言不一致时执行翻译。真正「当用户语言与角色语言不同时翻译」的逻辑位于 REST 层（例如 `main_routers/characters_router.py`，它通过该服务的 `translate_dict()` 翻译角色档案字段）。

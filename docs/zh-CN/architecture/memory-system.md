...
| **语义记忆** | 混合索引：向量嵌入（本地 CPU ONNX 模型，见 `memory/embeddings.py` 的 `EmbeddingService`）+ BM25 | 永久保留 | 相似度搜索 |
...
summary tier 模型（`DEFAULT_SUMMARY_MODEL`，默认 `qwen-plus`；可通过 `core_config.json` 或所选辅助 provider 的 `summary_model` 覆盖——而非通过环境变量）将原始对话压缩为存档用的摘要。

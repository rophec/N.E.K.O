...
当用户语言与角色配置语言不同时，自动翻译由 `TranslationService`（位于 `utils/language_utils.py`）执行，它依次回退到 googletrans → translatepy → 基于 LLM 的翻译。该过程运行在 REST 层（例如 `main_routers/characters_router.py`，用于翻译角色档案字段），而非 `LLMSessionManager` 的某个方法。

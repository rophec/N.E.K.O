...
| `mmd` | mmd_emotion_manager.html | mmd-*.js |
| `pngtuber` | (managed via model_manager.html) | pngtuber-core.js |
...
The backend also supports translation via `TranslationService` (`utils/language_utils.py`):

- Primary: `googletrans` library
- Fallback: `translatepy` library
- Final fallback: LLM-based translation

The session manager (`main_logic/core.py`) only exposes this service via the `_get_translation_service()` accessor — it does not itself translate on language mismatch. The actual "translate when the user's language differs from the character's language" logic runs in the REST layer (e.g. `main_routers/characters_router.py`, which translates character-profile fields via the service's `translate_dict()`).

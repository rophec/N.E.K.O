...
- **バスシステム** — `self.bus` でホストの状態を読み取り、その変化を購読するためのもの：`self.bus.messages` / `self.bus.events` / `self.bus.lifecycle` / `self.bus.conversations` / `self.bus.memory` の各名前空間が `.get(...)` でスナップショットを読み取れます。変化に反応するには、スナップショットのリストを watch して差分を購読します：`self.bus.events.get(...).watch(...).subscribe(on=...)`。publish/emit の API はありません — このバスは読み取り専用/リアクティブであり、汎用の pub/sub バスではありません。

...
| **セマンティックメモリ** | ハイブリッドインデックス：ベクトルEmbedding（ローカルCPU ONNXモデル、`memory/embeddings.py` の `EmbeddingService` を参照）+ BM25 | 永続 | 類似性検索 |
...
summary tier のモデル（`DEFAULT_SUMMARY_MODEL`、デフォルトは `qwen-plus`；環境変数ではなく `core_config.json` または選択中のアシストプロバイダーの `summary_model` で上書き可能）が、生の会話を保存用の要約に圧縮します。

...
| `pngtuber` | （model_manager.html 経由で管理） | pngtuber-core.js |
...
バックエンドも `TranslationService`（`utils/language_utils.py`）経由で翻訳をサポートしています：

- プライマリ: `googletrans` ライブラリ
- フォールバック: `translatepy` ライブラリ
- 最終フォールバック: LLM ベースの翻訳

セッションマネージャー（`main_logic/core.py`）は `_get_translation_service()` アクセサーでこのサービスを公開するだけで、言語不一致時の翻訳自体は行いません。実際の「ユーザーの言語がキャラクターの言語と異なる場合に翻訳する」ロジックは REST 層（例: `main_routers/characters_router.py`。サービスの `translate_dict()` でキャラクタープロフィールのフィールドを翻訳）にあります。

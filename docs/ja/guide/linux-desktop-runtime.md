...
## ファイルシステムエンコーディングと非 ASCII パス

Python バックエンドは、非 ASCII 文字を含むパス（多くはキャラクターディレクトリ、例えば `memory/玖儿/...`）を作成・オープンするとき、起動時に `UnicodeEncodeError` でクラッシュすることがあります。トレースバックは一見無害な `os.makedirs()` や `open()` を指します。

根本原因は、`open()` に渡すエンコーディングではなく、インタプリタの**ファイルシステムエンコーディング**（`sys.getfilesystemencoding()`）です。AppImage に内蔵された Python（あるいは任意の最小 Linux ランタイム）は、`C`/`POSIX` ロケールで起動し、PEP 538 のロケール強制が `C.UTF-8` ターゲットを見つけられないと `ascii` にフォールバックします。ファイルシステムエンコーディングが `ascii` になると、非 ASCII パスを伴うすべてのファイルシステム呼び出しが失敗します。注意点：

- システム側で `LANG=zh_CN.UTF-8` が正しく設定されていても**効きません**。起動シェルがロケールを内蔵インタプリタに伝播しないことが多いためです。
- `sys.getdefaultencoding()` は無関係です。Python 3 では常に UTF-8 であり、パス処理には影響しません。
- ファイルシステムエンコーディングはインタプリタ起動時に確定し、実行時には変更できません。

修正は **UTF-8 モード**（PEP 540）で、環境変数 `PYTHONUTF8=1` により有効化されます。これは実行時の環境変数なので、内蔵 Python の再ビルドは不要です。次の 2 層の防御がともに成立しているべきです：

1. **Electron シェル** —— `backend-runtime.js` が、spawn するバックエンドの環境に `PYTHONUTF8=1`（および `PYTHONIOENCODING=utf-8`）を設定します。これが AppImage 経路の主たる修正です。
2. **Python ランチャーの自己修復** —— `launcher.py` がエントリで `sys.getfilesystemencoding()` を確認し、UTF-8 でなければ `PYTHONUTF8=1` を設定して `os.execv` で自身を一度だけ再実行します（同一 PID なので、シェルのプロセス追跡には影響しません）。これは古いシェル、Steam ビルド、`backend-runtime.js` を経由しない直接起動をカバーします。

報告でこのクラッシュがなお出る場合は、まずそのビルドが両層を実際に備えているか確認してください。シェルの環境注入より前にビルドされた AppImage は、再ビルドするか、自己修復するランチャーに更新したときに初めてカバーされます。

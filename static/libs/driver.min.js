...
(function(global) {
    ...
    class Driver {
        constructor(options = {}) {
            ...
            this.pendingScrollCleanups = new Set();
        ...
        clearDelayedCallbacks() {
            Array.from(this.pendingScrollCleanups).forEach((cleanup) => cleanup());
            this.pendingScrollCleanups.clear();
            ...
            const proceed = () => {
                ...
                if (isInViewport) {
                    ...
                    const createAfterScroll = () => {
                        if (created || this.isDestroyed || !this.isActive) return;
                        created = true;
                        cleanupScroll();
                        this.createHighlight(element);
                        this.createPopover(element, popover, stepIndex);
                    ...
                    const onScroll = () => {
                        this.clearManagedTimeout(scrollTimer);
                        scrollTimer = this.setManagedTimeout(createAfterScroll, 150);
                    };

                    const cleanupScroll = () => {
                        window.removeEventListener('scroll', onScroll);
                        this.clearManagedTimeout(scrollTimer);
                        this.clearManagedTimeout(fallbackTimer);
                        this.pendingScrollCleanups.delete(cleanupScroll);
                    };

                    window.addEventListener('scroll', onScroll, { passive: true });
                    this.pendingScrollCleanups.add(cleanupScroll);
                    fallbackTimer = this.setManagedTimeout(createAfterScroll, 1200);
                }
            };
            ...
            if (!hasValidRect(element)) {
                ...
                const poll = () => {
                    ...
                    if (hasValidRect(element)) {
                        ...
                        this.setManagedTimeout(poll, 50);
                    }
                };
                poll();
            } else {
        ...
        cleanupDragHandlers() {
            ...
            this.dragHandlers.forEach(({ dragHandle, handleDragStart, handleDragMove, handleDragEnd }) => {
                if (dragHandle) {
                    dragHandle.removeEventListener('mousedown', handleDragStart);
                    dragHandle.removeEventListener('touchstart', handleDragStart, { passive: false });
                }

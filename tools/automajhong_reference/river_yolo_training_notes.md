...
uv run --with opencv-python-headless --with numpy python tools/automajhong_reference/run_tablecloth_warp_batch.py `
  --input-root "<source-images>" `
  --output-root "<source-images>\prelabel_20260626_v3\table_warp"
```
...
uv run --with opencv-python-headless --with numpy python tools/automajhong_reference/prepare_river_yolo_dataset.py `
  --input-root "<source-images>\prelabel_20260626_v3\table_warp" `
  --output-root "<source-images>\prelabel_20260626_v3\river_detection_draft" `
  --split train
```
...
```text
Input:  <source-images>
Warp:   <source-images>\prelabel_20260626_v3\table_warp
Labels: <source-images>\prelabel_20260626_v3\river_detection_draft
Warp success: 18 images
Warp failed:  1 image, 牌.png
River slots:  218 pre-label boxes
Review sheet:
  <source-images>\prelabel_20260626_v3\river_detection_draft\review\debug\00_slots_contact_sheet.png
Review CSV:
  <source-images>\prelabel_20260626_v3\river_detection_draft\review\river_review.csv
```
...
uv run --with opencv-python-headless --with numpy python tools/automajhong_reference/prepare_river_yolo_dataset.py `
  ...
  --output-root "<source-images>\yolo_datasets\river_detection_draft" `
  --split train
...
```text
<source-images>\yolo_datasets\river_detection_draft
├─ data.yaml
├─ images
│  ├─ train
│  └─ val
...
uv run --with ultralytics yolo detect train `
  model=yolo26n.pt `
  data="<source-images>\yolo_datasets\river_detection_draft\data.yaml" `
  epochs=3 `
  imgsz=800 `
  batch=2 `
  project="<source-images>\yolo_runs" `
...
```text
<source-images>\yolo_runs\river_detection_draft_yolo26_sanity\weights\best.pt
```
...
uv run --with ultralytics --with onnx --with onnxslim yolo export `
  model="<source-images>\yolo_runs\river_detection_draft_yolo26_sanity\weights\best.pt" `
  format=onnx `
  imgsz=800 `
  simplify=True
```
...
```text
<source-images>\yolo_runs\river_detection_draft_yolo26_sanity\weights\best.onnx
```
...
uv run --with ultralytics --with onnxruntime yolo predict `
  task=detect `
  model="<source-images>\yolo_runs\river_detection_draft_yolo26_sanity\weights\best.onnx" `
  source="<source-images>\yolo_datasets\river_detection_draft\images\val\11_river.png" `
  imgsz=800 `
  conf=0.001 `
  project="<source-images>\yolo_runs" `
...
```text
<source-images>\yolo_runs\river_detection_draft_onnx_predict
```
...
uv run python tools/automajhong_reference/build_reviewed_river_yolo34_dataset.py `
  --draft-root "<source-images>\yolo_datasets\river_detection_draft" `
  --output-root "<source-images>\yolo_datasets\river_tile34_reviewed" `
  --split train
```

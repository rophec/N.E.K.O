...
class TTSStream:
    ...
    def clone(self, 
        ...
        if self.voice is None:
            ...
            print("❌ [Clone] stream.voice 为 None，音色回退！请检查上游 set_voice 是否成功", flush=True)
            return None
    ...
    def reset(self, clear_voice: bool = False):
        """重置流状态（KV cache），可选保留音色锚点。

        Args:
            clear_voice: True 时清除 self.voice (向后兼容)；
                         False 时保留 voice，避免重复 set_voice 开销。
        """
        self.talker.clear_memory()
        ...
        if clear_voice:
            ...
            vinfo = f"voice={self.voice.text[:20]}" if self.voice else "voice=None"
            logger.info(f"扫 [Stream] 状态已重置 (talker/predictor KV cache, {vinfo} 保留)")
    ...
    def _set_voice_from_result(self, res: TTSResult) -> bool:
        ...
        # 如果缺少解码状态记忆 (例如从 JSON 加载)，预解码获取 final_state

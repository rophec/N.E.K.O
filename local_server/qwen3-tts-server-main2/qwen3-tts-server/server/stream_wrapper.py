    ...
    def _sync_synthesize(self, text, mode, speaker, language, instruct, config):
        ...
        try:
            ...
            if result and result.stats:
                ...
                llm_time = s.inference_only_time
                decode_time = s.total_decoder_time
                total_time = llm_time + decode_time
                tok_per_s = s.total_steps / total_time if total_time > 0 else 0
                rtf = total_time / audio_dur if audio_dur > 0 else 0
                summary = (
                    f"[TTS] text={text!r:.60s}  "
                    f"steps={s.total_steps}  "
                    f"audio={audio_dur:.2f}s  "
                    f"inference={total_time:.2f}s({llm_time:.2f}s+{decode_time:.2f}s)  "
                    f"RTF={rtf:.2f}x  "

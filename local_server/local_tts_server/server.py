...
class KokoroEngine:
    """Kokoro ONNX adapter with bundled model defaults."""

    name = "kokoro"
    hidden_voice_prefixes = ("zm_",)

    def synthesize(self, text: str, *, voice: str, speed: float) -> tuple[bytes, int]:
        from kokoro_cli import synthesize_to_pcm

        return synthesize_to_pcm(text, voice, speed)

    def list_voice_ids(self) -> list[str]:
        from kokoro_cli import DEFAULT_VOICES

        raw = os.getenv("LOCAL_TTS_KOKORO_VOICES", "").strip()
        if raw:
        ...
        return [
            ...
            if not voice_id.lower().startswith(self.hidden_voice_prefixes)
        ]


class CommandWavEngine:
    """Placeholder adapter for wrappers that still need external commands."""
...
def build_model_catalog() -> list[dict[str, str]]:
    ...
    kokoro_engine = ENGINES.get("kokoro")
    if isinstance(kokoro_engine, KokoroEngine):
        for voice_id in kokoro_engine.list_voice_ids():
            data.append(
                {
                    ...
                    "owned_by": "local_tts",
                    "name": voice_id,
                    "description": "Kokoro ONNX voice",
                }
            )

    if "tone" in ENGINES:
        for voice_id in ("tone:default", "tone:high"):
            data.append(
                {
                    "id": voice_id,
                    "object": "model",
                    ...
                    "description": "Built-in smoke-test voice",
                }
            )

    if not data:
        for name in sorted(ENGINES.keys()):
            data.append({"id": name, "object": "model", "owned_by": "local_tts", "name": name})
    return data
...
async def health():
    return {
        ...
        "default_model": DEFAULT_MODEL,
        "models": [item["id"] for item in build_model_catalog()],
    }

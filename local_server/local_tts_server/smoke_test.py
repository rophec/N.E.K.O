...
async def run_all(ws_url: str, http_base: str) -> int:
    # tone engine cases are the protocol baseline
    baseline_cases = [
        ("tone:default", 1.0, "Smoke test short sentence."),
        ("tone:high", 1.2, "Speed test with high voice."),
    ]

    # extra cases — only run if engine is both registered AND configured
    extra_cases = [
        ("piper:zh_CN-huayan-medium", 1.0, "Piper voice format test."),
        ("kokoro:af_heart", 1.0, "Kokoro English voice test."),
        ("kokoro:zf_xiaobei", 1.0, "Kokoro Chinese voice test."),
        ("melotts:zh", 1.0, "MeloTTS voice format test."),
        ("chattts:default", 1.0, "ChatTTS voice format test."),
    ]

    # Engines that are registered but need external config to actually work.
    # Skip them unless the relevant env var is set.
    env_required = {
        "piper": "LOCAL_TTS_PIPER_MODEL",
        "melotts": "LOCAL_TTS_MELOTTS_CMD",
        "chattts": "LOCAL_TTS_CHATTTS_CMD",
    }

    registered = fetch_engines(http_base)
    available = set()
    for eng in registered:
        env_key = env_required.get(eng)
    ...
    async def run_one(voice: str, speed: float, text: str, required: bool) -> None:
        ...
        if res["success"]:
            passed += 1
            print(f"  OK  PCM={res['pcm_bytes']} bytes")
        else:
            failed += 1

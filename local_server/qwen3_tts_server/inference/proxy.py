...
class DecoderProxy:
    ...
    def decode(self, input: Union[np.ndarray, TTSResult], task_id="default", is_final: bool = False,
        ...
        if isinstance(input, TTSResult):
            if input.ref_codes is not None and len(input.ref_codes) > 0 and input.final_state is None:
                input.final_state = self.prepare_continuation_state(
                    input.ref_codes,
                    task_id=f"{task_id}_ref_continuation",
                )
    ...
    def prepare_continuation_state(self, codes: np.ndarray, task_id: Optional[str] = None):
        """Return DecoderState after consuming an unfinished reference prefix."""
        task_id = task_id or f"reference_continuation_{self.task_counter}"
        self.task_counter += 1
        self.results[task_id] = []
        self.events[task_id] = threading.Event()
        self.active_decoder_tasks.add(task_id)
        self.decoder_idle.clear()
        self.codes_q.put(DecodeRequest(
            task_id=task_id,
            msg_type="PREPARE_STATE",
            codes=np.asarray(codes, dtype=np.int64),
            is_final=False,
        ))

        completed = self.events[task_id].wait(timeout=30.0)
        responses = self.results.get(task_id, [])
        result = DecodeResult(responses=responses)

        self.results.pop(task_id, None)
        self.events.pop(task_id, None)
        self.streaming_results.pop(task_id, None)
        self.active_decoder_tasks.discard(task_id)
        if not self.active_decoder_tasks:
            self.decoder_idle.set()

        if not completed or result.final_state is None:
            raise RuntimeError("参考声音的 Decoder 续读状态准备失败")

...
def handle_prepare_state(req: DecodeRequest, decoder, response_queue):
    """Decode reference codes as an unfinished prefix and return its state.

    Voice-clone ICL is a continuation task. Finalizing the reference prefix
    changes the decoder's ``is_last`` path and discards its latent tail, so the
    generated speech no longer starts from the same state as decoding
    ``reference_codes + generated_codes`` in one pass.
    """
    codes = np.array(req.codes, dtype=np.int64) if req.codes is not None else np.zeros((0, 16), dtype=np.int64)
    if codes.ndim == 1:
        codes = codes.reshape(-1, 16)

    try:
        t0 = time.time()
        _, state = decoder.decode(codes, state=req.state, is_final=False)
        response_queue.put(DecoderResponse(
            msg_type="FINISH",
            task_id=req.task_id,
            index=1,
        ...
        import traceback
        print(f"[DecoderWorker] 参考续读状态准备异常: {e}")
        traceback.print_exc()
        response_queue.put(DecoderResponse(msg_type="FINISH", task_id=req.task_id))
...
def decoder_worker_proc(codes_queue, pcm_queue, decoder_onnx_path, onnx_provider="CPU", chunk_size=12, record_queue=None):
    ...
    try:
        while True:
            ...
            elif req.msg_type == "PREPARE_STATE":
                handle_prepare_state(req, decoder, pcm_queue)

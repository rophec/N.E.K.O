...
def _llama_bin_dir() -> Path:
    configured = os.environ.get("QWEN3_TTS_LLAMA_BIN_DIR", "").strip()
    if configured:
...
def bind_llama_lib():
    ...
    global llama_decode, llama_get_logits, llama_get_logits_ith, llama_get_embeddings, llama_set_embeddings, llama_get_embeddings_ith, llama_tokenize
...
def init():
    ...
    lib_dir = _llama_bin_dir()
    if not lib_dir.is_dir():
        raise FileNotFoundError(
            f"llama.cpp binary directory not found: {lib_dir}. "
            "Set QWEN3_TTS_LLAMA_BIN_DIR or populate inference/bin."
        )
...
class LlamaContext:
    ...
    def __init__(self, model, n_ctx=2048, n_batch=2048, n_ubatch=512, n_seq_max=1, 
        ...
        # 新版 llama.cpp 需要显式启用 embeddings 输出
        if embeddings:
            llama_set_embeddings(self.ptr, True)
    ...
    def get_embeddings(self):
        """获取所有启用了 logits 的 token 的 embedding 输出（连续内存）。"""

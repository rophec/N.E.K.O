...
class MahjongCoachConfig:
    ...
    # Product gate, set by the distributed plugin at startup.  Keeping this
    # separate from the legacy presentation preference lets the old strategy
    # library remain testable without making it reachable from the product UI.
    companion_only_runtime: bool = False
...
def _valid_inference_provider(value: Any) -> str:
    ...
    if provider in {"speed", "fast", "directml", "dml", "gpu"}:
        return "speed"
    return "memory"

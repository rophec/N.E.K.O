...
def _load_onnx_session(
    ...
    _YOLO_RUNTIME_STATE["runtime_version"] = str(getattr(ort, "__version__", "") or "")
    _YOLO_RUNTIME_STATE["runtime_origin"] = str(getattr(ort, "__file__", "") or "")
...
def reset_yolo26_accelerator_session() -> None:
    """Forget a degraded speed session so the next warmup retries the GPU.

    A ``speed`` session may legitimately fall back to CPU when CUDA/DirectML
    initialization fails.  Keeping that CPU session for the current retry
    window avoids rebuild loops, but it must not remain cached forever after a
    transient driver or device failure.
    """
    ...
    _YOLO_RUNTIME_STATE.update(
        {
            ...
            "requested_providers": [],
            "actual_providers": [],
            "fallback_reason": "accelerator_retry_pending",
            "updated_at": time.time(),
        }
    )
...
def warmup_yolo26_runtime(
    *,
    ...
    # DirectML may finish session construction before its command queues and
    # kernels have reached steady state.  A single black inference therefore
    # still left the first one or two real Mahjong frames paying multi-second
    # setup costs on some machines.  Prime a few fixed-shape passes here while
    # the player is still on the mode-selection screen.
    probe = Image.new("RGB", backend.input_size, (96, 112, 128))
    try:
        for _pass in range(_WARMUP_INFERENCE_PASSES):
    ...
    runtime = dict(_YOLO_RUNTIME_STATE)
    fallback_reason = str(runtime.get("fallback_reason") or "")
    return {
        # A successfully created CPU session is usable, but it is not a
        # successful GPU warmup.  Keep those states distinct so the UI cannot
        # claim "GPU ready" after ONNX Runtime silently falls back to CPU.
        "status": "ready" if accelerated or backend.inference_provider != "speed" else "fallback",
        "reason": fallback_reason,
        "elapsed_ms": round((time.perf_counter() - started) * 1000.0, 1),
        "providers": providers,
        "available_providers": list(runtime.get("available_providers") or []),
        "requested_providers": list(runtime.get("requested_providers") or []),
        ...
        "warmup_passes": _WARMUP_INFERENCE_PASSES,
    }

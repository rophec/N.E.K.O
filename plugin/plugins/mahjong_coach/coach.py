...
_RIVER_MISSING_RETRACT_FRAMES = 3
_RIVER_MAX_RETRACTION_FRACTION = 0.25
_RIVER_MATCH_MIN_IOU = 0.25
_RIVER_OWNER_MATCH_MIN_IOU = 0.55
...
class RoundCoachEngine:
    def __init__(
        ...
        *,
        ...
        self._river_last_self_meld_count: int | None = None
        self._river_last_opponent_meld_count: int | None = None
    ...
    def reset_round(self, round_id: str = "default") -> RoundCoachState:
        ...
        self._river_last_self_meld_count = None
        self._river_last_opponent_meld_count = None
    ...
    def analyze_frame(
        ...
        *,
        ...
        if fp and not fp["action_changed"] and not fp["hand_changed"] and self._new_round_candidate_frames == 0:
    ...
    def _detect_hand(self, path: Path | None, *, min_hand_tiles: int | None = None) -> FastHandResult:
        ...
        if self._uses_yolo26_tiles():
            ...
            if yolo_result.ok:
                return yolo_result.to_hand_result(min_hand_tiles=max(1, int(min_hand_tiles or self._min_hand_tiles_for_scan())))
    ...
    def _detect_river(self, path: Path | None) -> RiverStateResult:
        ...
        if self._uses_yolo26_tiles():
            yolo_result = self._detect_yolo26_table(path)
            if yolo_result.ok:
                return yolo_result.to_river_result()
    ...
    def _track_river_for_frame(
        ...
        *,
        ...
        call_buttons: list[str] | None = None,
    ) -> RiverStateResult:
        ...
        if self._uses_live_river_tracking() and self.state.river_tracking_initialized:
            ...
            if delta_result.ok:
                merged = self._merge_incremental_river(
                    delta_result,
                    preserve_new_history=bool(call_buttons),
                )
    ...
    def _merge_incremental_river(
        self,
        delta_result: RiverStateResult,
        *,
        preserve_new_history: bool = False,
    ) -> RiverStateResult:
        merged_piles = {
        ...
        for player, items in delta_result.discard_piles.items():
            ...
            for item in items:
                ...
                if slot_key in known_slots:
                    continue
                new_item = _prepare_new_river_item(item, confirmations=1)
                if preserve_new_history:
                    new_item["preserve_history"] = True
                    new_item["history_reason"] = "call_window_discard"
    ...
    def _reconcile_full_river(
        ...
        *,
        ...
        merged_piles = {
            player: [_normalize_cached_river_item(item) for item in items]
            for player, items in self.state.last_discard_piles.items()
        }
        initial_lengths = {player: len(items) for player, items in merged_piles.items()}
        used_indices: dict[str, set[int]] = {player: set() for player in merged_piles}
        matched_refs: set[tuple[str, int]] = set()
        deferred_removals: set[tuple[str, int]] = set()
        ...
        call_observed_items: list[dict[str, Any]] = []
        call_players = _call_candidate_players(call_buttons or [])
        meld_growth = self._river_meld_growth(snapshot)

        for player, current_items in snapshot.discard_piles.items():
            target = merged_piles.setdefault(player, [])
            player_used_indices = used_indices.setdefault(player, set())
            latest_current_turn = max(
                [int(item.get("turn_index") or 0) for item in current_items if isinstance(item, dict)] or [0]
            )
            for raw_item in current_items:
                if not isinstance(raw_item, dict) or not str(raw_item.get("tile") or ""):
                    continue
                current = dict(raw_item)
                match_index = _match_river_item_index(target, current, player_used_indices)
                if match_index is None:
                    owner_match = _match_river_item_other_player(
                        merged_piles,
                        current,
                        current_player=player,
                        used_refs=matched_refs,
                        initial_lengths=initial_lengths,
                    )
                    if owner_match is not None:
                        previous_player, previous_index = owner_match
                        cached = merged_piles[previous_player][previous_index]
                        matched_refs.add(owner_match)
                        used_indices.setdefault(previous_player, set()).add(previous_index)
                        candidate_key = _river_candidate_key("move", previous_player, cached)
                        active_candidate_keys.add(candidate_key)
                        confirmations = self._advance_river_candidate(candidate_key, "move", player, current)
                        if confirmations < _RIVER_CORRECTION_CONFIRM_FRAMES:
                            continue

                        moved = _refresh_river_item(cached, current)
                        moved["player"] = player
                        moved["turn_index"] = int(current.get("turn_index") or 0) or _next_river_turn_index(target)
                        moved["corrected_player_from"] = previous_player
                        moved["owner_correction_confirmations"] = confirmations
                        moved["correction_source"] = str(
                            current.get("source") or snapshot.reason or "full_rescan"
                        )
                        target.append(moved)
                        deferred_removals.add(owner_match)
                        migrated.append(
                            {
                                "tile": moved.get("tile"),
                                "from_player": previous_player,
                                "to_player": player,
                                "turn_index": moved.get("turn_index"),
                                "confidence": moved.get("confidence"),
                            }
                        )
                        if player in call_players and int(current.get("turn_index") or 0) == latest_current_turn:
                            call_observed_items.append(moved)
                    ...
                    if confirmations < _RIVER_CORRECTION_CONFIRM_FRAMES:
                        continue
                    new_item = _prepare_new_river_item(current, confirmations=confirmations)
                    new_item["player"] = player
                    new_item["turn_index"] = _next_river_turn_index(target)
                    target.append(new_item)
                    appended.append(dict(new_item))
                    if player in call_players and int(current.get("turn_index") or 0) == latest_current_turn:
                        call_observed_items.append(new_item)
                    self._river_correction_candidates.pop(candidate_key, None)
                    continue

                player_used_indices.add(match_index)
                matched_refs.add((player, match_index))
                ...
                if str(cached.get("tile") or "") == str(current.get("tile") or ""):
                    refreshed = _refresh_river_item(cached, current)
                    target[match_index] = refreshed
                    if player in call_players and int(current.get("turn_index") or 0) == latest_current_turn:
                        call_observed_items.append(refreshed)
                    self._river_correction_candidates.pop(candidate_key, None)
                    continue
                ...
                if player in call_players and int(current.get("turn_index") or 0) == latest_current_turn:
                    call_observed_items.append(replacement)
                corrected.append(
                    {
                        "player": player,
        ...
        initial_refs = {
            (player, index)
            for player, length in initial_lengths.items()
            for index in range(length)
        }
        missing_refs = sorted(initial_refs - matched_refs - deferred_removals)
        retractable_missing_refs: list[tuple[str, int]] = []
        for player, index in missing_refs:
            item = merged_piles[player][index]
            item["currently_visible"] = False
            if bool(item.get("preserve_history")) or meld_growth:
                item["preserve_history"] = True
                item["history_reason"] = str(item.get("history_reason") or "meld_growth")
                item["missing_confirmations"] = 0
                preserved_missing.append(
                    {
                        "player": player,
                        "turn_index": item.get("turn_index"),
                        "tile": item.get("tile"),
                        "reason": item.get("history_reason"),
                    }
                )
                continue
            retractable_missing_refs.append((player, index))

        unprotected_initial_count = sum(
            not bool(merged_piles[player][index].get("preserve_history"))
            for player, index in initial_refs
        )
        max_safe_retractions = max(1, int(unprotected_initial_count * _RIVER_MAX_RETRACTION_FRACTION))
        stable_absence_audit = 0 < len(retractable_missing_refs) <= max_safe_retractions
        # 中文：只累计小范围、连续的缺失证据；一次消失太多牌时视为菜单遮挡或整帧识别失败。
        # English: Count only small, consecutive gaps; broad loss is treated as obstruction or frame failure.
        if stable_absence_audit:
            for player, index in retractable_missing_refs:
                item = merged_piles[player][index]
                candidate_key = _river_candidate_key("missing", player, item)
                active_candidate_keys.add(candidate_key)
                confirmations = self._advance_river_candidate(candidate_key, "missing", player, item)
                item["missing_confirmations"] = confirmations
                if confirmations < _RIVER_MISSING_RETRACT_FRAMES:
                    continue
                deferred_removals.add((player, index))
                retracted.append(
                    {
                        "player": player,
                        "turn_index": item.get("turn_index"),
            ...
            for player, index in retractable_missing_refs:
                merged_piles[player][index]["missing_confirmations"] = 0

        if call_observed_items:
            call_item = min(
                call_observed_items,
                key=lambda item: (int(item.get("observation_count") or 0), -float(item.get("confidence") or 0.0)),
            )
            call_item["preserve_history"] = True
            call_item["history_reason"] = "call_window_discard"

        removal_indices: dict[str, set[int]] = {}
        for player, index in deferred_removals:
            removal_indices.setdefault(player, set()).add(index)
        for player, indices in removal_indices.items():
            merged_piles[player] = [
                item
                for index, item in enumerate(merged_piles.get(player, []))
                if index not in indices
            ]

        for target in merged_piles.values():
            target.sort(key=lambda item: int(item.get("turn_index") or 0))
        ...
        hints = {
            **snapshot.analysis_hints,
            ...
            "river_corrected_count": len(corrected) + len(migrated) + len(retracted),
            "river_class_corrected_count": len(corrected),
            "river_owner_migrated_count": len(migrated),
            "river_retracted_count": len(retracted),
            "river_preserved_missing_count": len(preserved_missing),
            "river_pending_corrections": len(self._river_correction_candidates),
            "river_correction_confirm_frames": _RIVER_CORRECTION_CONFIRM_FRAMES,
            "river_retraction_confirm_frames": _RIVER_MISSING_RETRACT_FRAMES,
            "river_correction_events": corrected,
            "river_owner_correction_events": migrated,
            "river_retraction_events": retracted,
            "river_preserved_missing_events": preserved_missing,
            "new_discard_count": len(appended),
            "cached_discard_count": len(visible_tiles),
            "current_visible_discard_count": _river_snapshot_count(snapshot.discard_piles),
            "river_meld_growth_guard": meld_growth,
            "river_absence_audit_stable": stable_absence_audit,
            "river_absence_audit_limit": max_safe_retractions,
        }
        return RiverStateResult(
            ok=True,
            discard_piles=merged_piles,
            visible_tiles=visible_tiles,
            confidence=round(sum(confidences) / len(confidences), 4) if confidences else 0.0,
            reason="river_corrected" if corrected or migrated or retracted else snapshot.reason,
    ...
    def _river_meld_growth(self, snapshot: RiverStateResult) -> bool:
        self_meld_count, opponent_meld_count = _river_snapshot_meld_counts(snapshot)
        growth = False
        if self_meld_count is not None:
            growth = growth or (
                self._river_last_self_meld_count is not None
                and self_meld_count > self._river_last_self_meld_count
            )
            self._river_last_self_meld_count = self_meld_count
        if opponent_meld_count is not None:
            growth = growth or (
                self._river_last_opponent_meld_count is not None
                and opponent_meld_count > self._river_last_opponent_meld_count
            )
            self._river_last_opponent_meld_count = opponent_meld_count
        return growth

    def _sync_river_meld_counts(self, river_result: RiverStateResult) -> None:
        self_meld_count, opponent_meld_count = _river_snapshot_meld_counts(river_result)
        if self_meld_count is not None:
            self._river_last_self_meld_count = self_meld_count
        if opponent_meld_count is not None:
            self._river_last_opponent_meld_count = opponent_meld_count
    ...
    def _call_window_river(self, path: Path | None, buttons: list[str]) -> tuple[RiverStateResult, str]:
        ...
        river_result = self._track_river_for_frame(
            path,
            "call_window_river_delta",
            call_buttons=buttons,
        )
        if not river_result.ok:
            return river_result, ""
        return river_result, _claimed_tile_from_river_delta(previous_piles, river_result.discard_piles, buttons)
    ...
    def _detect_riichi_players(self, path: Path | None) -> list[str]:
        ...
        if self._uses_yolo26_tiles():
            yolo_result = self._detect_yolo26_table(path)
            if yolo_result.ok:
                detected.update(yolo_result.riichi_players)
        result = detect_riichi_sticks(path)
        detected.update(self._riichi_players_from_stick_counter(result))
        confirmed: list[str] = []
    ...
    def _detect_yolo26_table(self, path: Path) -> Yolo26TableStateResult:
        if self._last_yolo26_path == path and self._last_yolo26_result is not None:
            return self._last_yolo26_result
        result = detect_yolo26_table_state_path(path)
    ...
    def _remember_river(self, river_result: RiverStateResult) -> None:
        ...
        self._sync_river_meld_counts(river_result)
    ...
    def _defense_suggestion(
        ...
        defense = _defense_options(held_tiles, riichi_players, piles)
...
def _normalize_cached_river_item(item: dict[str, Any]) -> dict[str, Any]:
    normalized = dict(item)
    normalized.setdefault("observation_count", 1)
    normalized.setdefault("missing_confirmations", 0)
    normalized.setdefault("currently_visible", True)
    return normalized


def _prepare_new_river_item(item: dict[str, Any], *, confirmations: int) -> dict[str, Any]:
    prepared = dict(item)
    prepared["observation_count"] = max(1, int(confirmations or 1))
    prepared["missing_confirmations"] = 0
    prepared["currently_visible"] = True
    return prepared


def _next_river_turn_index(items: list[dict[str, Any]]) -> int:
    return max([int(item.get("turn_index") or 0) for item in items if isinstance(item, dict)] or [0]) + 1
...
def _match_river_item_other_player(
    cached_piles: dict[str, list[dict[str, Any]]],
    current: dict[str, Any],
    *,
    current_player: str,
    used_refs: set[tuple[str, int]],
    initial_lengths: dict[str, int],
) -> tuple[str, int] | None:
    """Find a high-overlap historical record that was assigned to another player."""
    current_bbox = _river_bbox(current)
    if current_bbox is None:
        return None
    scored: list[tuple[float, str, int]] = []
    for player, items in cached_piles.items():
        if player == current_player:
            continue
        for index, cached in enumerate(items[: initial_lengths.get(player, 0)]):
            if (player, index) in used_refs or bool(cached.get("preserve_history")):
                continue
            cached_bbox = _river_bbox(cached)
            if cached_bbox is None:
                continue
            scored.append((_river_bbox_iou(current_bbox, cached_bbox), player, index))
    if not scored:
        return None
    score, player, index = max(scored)
    if score < _RIVER_OWNER_MATCH_MIN_IOU:
        return None
    return player, index
...
def _refresh_river_item(cached: dict[str, Any], current: dict[str, Any]) -> dict[str, Any]:
    ...
    refreshed["observation_count"] = max(1, int(cached.get("observation_count") or 1)) + 1
    refreshed["missing_confirmations"] = 0
    refreshed["currently_visible"] = True
    refreshed.pop("history_status", None)
    return refreshed
...
def _river_snapshot_count(discard_piles: dict[str, list[dict[str, Any]]]) -> int:
    return sum(
        1
...
def _river_snapshot_meld_counts(snapshot: RiverStateResult) -> tuple[int | None, int | None]:
    hints = snapshot.analysis_hints if isinstance(snapshot.analysis_hints, dict) else {}
    ...
    return read_count("yolo26_self_meld_count"), read_count("yolo26_opponent_meld_count")
...
def _claimed_tile_from_river_delta(
    ...
    """Return the newly discarded tile only when the river delta is unambiguous."""
    candidate_players = _call_candidate_players(buttons)
    ...
    unique = _merge_unique_tiles(additions)
    return unique[0] if len(unique) == 1 else ""


def _call_candidate_players(buttons: list[str]) -> set[str]:
    normalized_buttons = set(_normalize_buttons(buttons))
    if not normalized_buttons.intersection(CALL_BUTTONS):
        return set()
    if "chi" in normalized_buttons and not normalized_buttons.intersection({"pon", "kan"}):
        return {"left_opponent"}
    return {"left_opponent", "top_opponent", "right_opponent"}
...
def rank_discard_decisions(
    ...
    *,
    ...
    visible_tiles = _visible_tiles_for_plan(river_result)
    plan = build_round_plan(tiles, config, visible_tiles=visible_tiles, open_melds=open_melds)
    ...
    for option in options:
        ...
        for label, player_key in opponents:
            ...
            risk, basis = _tile_risk_against_player(canonical, discards, visible_counts, dora_tiles, known_player=bool(player_key))
            risk_by_player[label] = {"risk": risk, "basis": basis}
            risks.append(risk)
        defense_risk = max(risks, default=0.0)
        if defense_risk > 0 and len(risks) > 1:
            defense_risk = min(100.0, defense_risk + min(10.0, (len(risks) - 1) * 4.0))
    ...
    mode = "attack"
    if opponents:
        if float(best.get("defense_risk", 100.0)) <= 10.0:
            mode = "defense"
        elif int(best.get("shanten", 8)) <= 1 and float(best.get("defense_risk", 100.0)) <= 45.0:
            mode = "balanced"
        else:
            mode = "defense"
    return {
...
def _defense_weight(shanten: int, riichi_count: int, style: str) -> float:
    if riichi_count <= 0:
        return 0.0
    if shanten <= 0:
        weight = 0.35
...
def _discard_ranking_text(ranking: dict[str, Any]) -> str:
    ...
    mode_names = {"attack": "进攻排序", "balanced": "攻守兼顾", "defense": "防守排序"}
    ...
    return f"{mode_names.get(str(ranking.get('mode')), '攻守排序')}：{warning}" + "；".join(parts) + "。"

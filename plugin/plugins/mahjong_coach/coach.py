...
from .strategy.efficiency import analyze_hand_efficiency, calculate_shanten
from .strategy.risk import GenbutsuAssessment, assess_genbutsu
...
class RoundCoachEngine:
    ...
    def _remember_local_plan(self, plan: dict[str, Any], *, opening: bool = False) -> None:
        ...
        self.state.plan_source = "deterministic_efficiency+heuristic_route"
        ...
        self.state.last_efficiency = dict(efficiency) if isinstance(efficiency, dict) else {}
        self.state.last_defense = {}
    ...
    def _defense_decision(
        ...
        player_keys = [key for key in (_player_key(player) for player in riichi_players) if key]
        defense = assess_genbutsu(
            list(hand_result.hand_tiles) if hand_result.ok else self.state.last_hand_tiles,
            piles,
            player_keys,
        )
        self.state.last_defense = defense.to_dict()
        reason_codes = ["riichi_players_present", "genbutsu_assessed"]
        if defense.held_safe_against_all:
            reason_codes.append("common_genbutsu_in_hand")
        if defense.missing_players:
            reason_codes.append("riichi_river_missing")
        return CoachDecision(
            ...
            analysis_source="deterministic_genbutsu",
        )
    ...
    def _defense_suggestion(
        ...
        assessment: GenbutsuAssessment | None = None,
    ) -> str:
        piles = river_result.discard_piles if river_result.ok else self.state.last_discard_piles
        player_keys = [_player_key(player) for player in riichi_players]
        player_keys = [key for key in player_keys if key]
        assessment = assessment or assess_genbutsu(hand_tiles or self.state.last_hand_tiles, piles, player_keys)
        if assessment.held_safe_against_all:
            safe_text = _tile_list_in_order(list(assessment.held_safe_against_all)[-8:])
            return f"防守优先：手里的 {safe_text} 是所有已识别立直家的共同现物；没有共同现物时再评估筋、壁和其他风险。"
        if len(player_keys) > 1 and assessment.safe_against_all:
            all_safe_text = _tile_list_in_order(list(assessment.safe_against_all)[-8:])
            return f"防守优先：共同现物有 {all_safe_text}，但手里没有；下一步需要评估筋、壁和统计放铳风险。"
        if len(player_keys) > 1 and assessment.safe_by_player:
            # 多家立直必须取现物交集，不能把只对一家安全的牌推荐为通用安全牌。
            # Multiple riichi requires intersection, never a union of per-player genbutsu.
            return "防守优先：各立直家的现物没有共同交集，不能把只对一家安全的牌当成通用安全牌。"
...
def build_round_plan(
    ...
    *,
    ...
    visible_counts = _visible_counts(visible_tiles or [])
...
def _efficiency_analysis(
    ...
    *,
    ...
    # The exact engine enumerates every distinct physical discard. The legacy
    # heuristic is used only as a final tie-breaker so it cannot override a
    # better shanten or ukeire result.
    river_tiles = list(visible_counts.elements())
    ...
    all_options = payload.get("all_discard_options")
    if isinstance(all_options, list):
        all_options.sort(
            key=lambda item: (
                int(item.get("shanten", 8)),
                -int(item.get("effective_count", 0)),
                -int(item.get("effective_types", 0)),
                _discard_score(
        ...
        payload["discard_options"] = all_options[:5]
    payload["calculation_source"] = "strategy.efficiency/analyze_hand_efficiency"
    return payload
...
def _best_hand_shanten(tiles: list[str], *, open_melds: int = 0) -> dict[str, Any]:
    result = calculate_shanten(tiles, open_melds=open_melds)
    return {"path": result.path, "shanten": result.shanten}

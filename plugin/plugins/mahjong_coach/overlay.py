...
_SWITCH_STYLE_LABEL = "伙伴设置"
_CONFIG_STYLE_HINT = "只观察公开牌局事件并负责搭话 · 不提供打牌建议。"
...
def _normalize_panel_mode(value: Any) -> str:
    # The distributed product is companion-only.  Old full-strategy
    # preferences are deliberately migrated to the compact banter panel.
    return "compact"
...
def _normalize_guidance_mode(value: Any) -> str:
    # Keep accepting the legacy field for profile compatibility, but never
    # expose or reactivate live strategy guidance.
    return "companion"
...
def _should_auto_show_running_panel(
    *,
    started: bool,
    live_running: bool,
    ...
    """Only leave settings automatically when the user has not opened it manually."""

    return bool(started and live_running and config_visible and not settings_pinned)
    ...
    *,
    ...
    if not live_running:
        if str(last_error or "").strip():
            return "未能开始观察", str(last_error).strip(), True
        return (
            "功能已开启 · 等待开始捕获",
            "当前尚未开始捕获，请点击上方“开始观察牌局”。",
            True,
        )
    ...
    return "猫娘已经在看你的屏幕啦", "接下来会陪你打牌助阵哦！", False


def _win32_topbar_rects(
    width: int,
) -> tuple[
    ...
    """Return close/minimize/style hit boxes in client coordinates."""

    client_width = max(1, int(width))
    close_rect = (max(0, client_width - 44), 0, client_width, 60)
    minimize_rect = (
        max(0, client_width - 88),
        0,
        max(0, client_width - 44),
        60,
    )
    switch_rect = (
        max(0, client_width - 228),
        0,
        max(0, client_width - 88),
        60,
    )
    return close_rect, minimize_rect, switch_rect
...
def _win32_topbar_action(mode: str, x: int, y: int, width: int) -> str:
    close_rect, minimize_rect, switch_rect = _win32_topbar_rects(width)
    if _point_in_rect(close_rect, x, y):
        return "close"
    if mode != "detail" and _point_in_rect(minimize_rect, x, y):
        return "minimize"
    if mode != "strategy":
        return ""
...
def _win32_config_action(x: int, y: int, width: int) -> tuple[str, str]:
    ...
    rows = (
        ("start", "riichi", (22, 154, client_width - 22, 244)),
        ("companion_toggle", "", (midpoint + 80, 330, midpoint + 230, 407)),
    )
...
class CoachOverlayController:
    ...
    def minimize(self) -> None:
        """Minimize only the surface; keep capture and companion work alive."""

        thread = self._thread
        if thread is None or not thread.is_alive():
            return
        self.window_visible = False
        self._queue.put({"cmd": "minimize"})

    def restore(self) -> None:
        """Restore an existing surface without restarting its live session."""

        thread = self._thread
        if thread is None or not thread.is_alive():
            return
        self._queue.put({"cmd": "restore"})
    ...
    def _run_win32(self) -> None:
        ...
        state: dict[str, Any] = {
            ...
            # The low-frequency novelty mode remains readable for config
            # compatibility, but is intentionally not exposed by the product UI.
            "absurd_banter_enabled": False,
            "live_running": False,
            "live_status": "stopped",
        ...
        def _draw_top_buttons(hdc: int, width: int, *, detail: bool = False) -> None:
            ...
            minimize_left, _, minimize_right, _ = minimize_rect
            if not detail:
                _draw_text(
                    hdc,
                    "—",
                    (minimize_left + 9, 11, minimize_right - 3, 48),
                    color=win32api.RGB(102, 102, 102),
                    small=True,
                )
        ...
        def _paint_main(hwnd: int, hdc: int, rect: tuple[int, int, int, int]) -> None:
            ...
            if state["mode"] == "config":
                _draw_text(hdc, "吐槽伙伴", (24, 20, width - 100, 54), color=win32api.RGB(30, 30, 30), custom_font=candidate_font)
                _draw_text(hdc, "观察公开牌局事件，交给 N.E.K.O 自然搭话", (24, 60, width - 80, 86), color=win32api.RGB(102, 102, 102), small=True)
                mid = width // 2
                ...
                win32gui.FillRect(hdc, (22, 154, width - 22, 244), accent_brush)
                _draw_text(hdc, "开始观察牌局", (mid - 105, 184, mid + 130, 218), color=win32api.RGB(255, 255, 255), custom_font=candidate_font)
                _draw_text(hdc, "搭话开关", (24, 286, mid + 100, 316), color=win32api.RGB(102, 102, 102), small=True)
                companion_enabled = bool(state["neko_companion_enabled"])
                win32gui.FillRect(hdc, (mid - 230, 316, mid + 230, 406), card_brush)
                win32gui.FrameRect(hdc, (mid - 230, 316, mid + 230, 406), border_brush)
                _draw_text(hdc, companion_title, (mid - 210, 327, mid + 70, 351), color=win32api.RGB(30, 30, 30), small=True)
                companion_card_detail = (
                    "关键事件出现后会主动搭话。"
                    if companion_enabled
                    else "伙伴搭话已停止，可随时重新开启。"
                )
                _draw_text(hdc, companion_card_detail, (mid - 210, 354, mid + 75, 385), color=win32api.RGB(102, 102, 102), small=True)
                companion_toggle_brush = accent_brush if companion_enabled else white_brush
                win32gui.FillRect(hdc, (mid + 80, 334, mid + 215, 388), companion_toggle_brush)
                win32gui.FrameRect(hdc, (mid + 80, 334, mid + 215, 388), accent_brush if companion_enabled else border_brush)
                _draw_text(
                    hdc,
                    "已开启  ✓" if companion_enabled else "已禁用",
                    (mid + 105, 349, mid + 205, 379),
                    color=win32api.RGB(255, 255, 255) if companion_enabled else win32api.RGB(70, 70, 70),
                    small=True,
                )
                _draw_text(hdc, _CONFIG_STYLE_HINT, (120, 430, width - 90, 472), color=win32api.RGB(102, 102, 102), small=True)
                return
            win32gui.FillRect(hdc, (18, 16, 150, 54), accent_brush)
            _draw_text(hdc, "吐槽伙伴", (34, 25, 144, 51), color=win32api.RGB(255, 255, 255), small=True)
            runtime_title, runtime_detail, runtime_warning = _companion_runtime_copy(
                enabled=bool(state["neko_companion_enabled"]),
                live_running=bool(state["live_running"]),
                delivery_stage=str(state["companion_delivery_stage"]),
                last_error=str(state["companion_last_error"]),
            ...
            runtime_brush = (
                warning_brush
                if runtime_warning or not bool(state["neko_companion_enabled"])
                else success_brush
            )
            win32gui.FillRect(hdc, (18, 70, width - 18, 132), runtime_brush)
            _draw_text(hdc, f"● {runtime_title}", (38, 78, width - 38, 103), color=win32api.RGB(255, 255, 255), small=True)
            _draw_text(hdc, runtime_detail, (38, 104, width - 38, 128), color=win32api.RGB(255, 255, 255), small=True)
            win32gui.FillRect(hdc, (18, 144, width - 18, height - 18), card_brush)
            panel_text = str(state["text"] or "").strip() or "吐槽伙伴正在等待牌局事件。"
            _draw_text(hdc, panel_text, (38, 164, width - 38, height - 32), color=win32api.RGB(30, 30, 30))
        ...
        def _paint_detail(hwnd: int, hdc: int, rect: tuple[int, int, int, int]) -> None:
            ...
            _draw_text(hdc, "牌局事件与识别诊断", (18, 14, width - 50, 40), color=win32api.RGB(30, 30, 30), small=True)
        ...
        def _restore_main() -> None:
            ...
            _resize_main(str(state["mode"]))
            # Windows may legally reject foreground activation when another
            # process owns the input lock.  The surface is already restored;
            # a denied focus request must not tear down the overlay thread.
            try:
        ...
        def _wnd_proc(hwnd: int, message: int, wparam: int, lparam: int) -> int:
            ...
            if message == win32con.WM_LBUTTONDOWN:
                ...
                if topbar_action == "close":
                    _close_main()
                    return 0
                if topbar_action == "minimize":
                    self.minimize()
                    return 0
                if topbar_action == "switch_style":
                    state["settings_pinned"] = True
                    _resize_main("config")
                    return 0
                if state["mode"] == "config":
                    ...
                    if config_action == "start":
                        ...
                        try:
                            if self._on_start is not None:
                                ...
                                state["companion_last_error"] = "当前只是面板预览，未连接 N.E.K.O 捕获后端。"
                        except Exception as exc:
                            self.last_error = f"button click failed: {exc}"
                            state["companion_last_error"] = self.last_error
                        win32gui.InvalidateRect(hwnd, None, True)
                        return 0
                    ...
                    if config_action == "panel_mode":
                        ...
                        if state["started"] and not state["settings_pinned"]:
            ...
            if message == win32con.WM_CLOSE:
                if hwnd == state["detail"]:
                    ...
                    _close_main()
                return 0
            if message == win32con.WM_SIZE and hwnd == state["main"]:
                self.window_visible = int(wparam) != int(win32con.SIZE_MINIMIZED)
                ...
                while True:
                    ...
                    if isinstance(item, dict):
                        ...
                        if command == "show_config":
                            ...
                            if _should_auto_show_running_panel(
                                started=bool(state["started"]),
                                live_running=bool(state["live_running"]),
                                settings_pinned=bool(state["settings_pinned"]),
                                config_visible=state["mode"] == "config",
                            ):
                                _resize_main("strategy")
                            detail_hwnd = int(state["detail"] or 0)
                            if detail_hwnd and win32gui.IsWindow(detail_hwnd):
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            minimize_btn.place(relx=1.0, x=-48, y=3, anchor="ne")

            config_btn = tk.Label(
                inner, text=_SWITCH_STYLE_LABEL, bg=_BTN_PRIMARY, fg="#ffffff",
                font=(_FONT, 15, "bold"), cursor="hand2", padx=18, pady=10,
            )
            config_btn.place(relx=1.0, x=-96, y=7, anchor="ne")

            def _on_minimize_click(_event: tk.Event | None = None) -> None:
                self.minimize()
            ...
            minimize_btn.bind("<Button-1>", _on_minimize_click)

            def _on_root_close_hit(event: tk.Event) -> None:
                width = root.winfo_width()
                if int(event.x) >= max(0, width - 56) and int(event.y) <= 48:
                    _on_close_click(event)
                elif int(event.x) >= max(0, width - 112) and int(event.y) <= 48:
                    _on_minimize_click(event)
            ...
            minimize_state = {"active": False}

            def _minimize_root() -> None:
                minimize_state["active"] = True
            ...
            def _on_root_map(event: tk.Event) -> None:
                if (
                    event.widget is root
                    and minimize_state["active"]
                    and root.state() != "iconic"
                ):
                    root.after_idle(_restore_root)

            root.bind("<Map>", _on_root_map, add="+")

            def _on_config_click(_event: tk.Event) -> None:
                try:
                    session_state["settings_pinned"] = True
                    show_config_mode()
                except Exception as exc:
            ...
            def show_detail_panel() -> None:
                ...
                tk.Label(head, text="牌局事件与识别诊断", bg=_BG, fg=_TEXT, font=(_FONT, 16, "bold")).pack(side="left")
            ...
            layout = {
                ...
                "guidance_mode": prefs["guidance_mode"],
                "neko_companion_enabled": prefs["neko_companion_enabled"],
                # Do not restore a previously enabled hidden novelty mode.
                "absurd_banter_enabled": False,
                "live_running": False,
                "live_status": "stopped",
                "companion_delivery_stage": "waiting_capture",
            ...
            session_state = {"started": False, "settings_pinned": False}
            ...
            def _make_btn(parent, text, style):
                ...
                def _on_click(_e):
                    session_state["settings_pinned"] = False
                    try:
                        if self._on_start is not None:
                            self._on_start(
                                style,
            ...
            def _make_panel_mode_btn(parent, text, mode):
                ...
                def _on_click(_e):
                    ...
                    if session_state["started"] and not session_state["settings_pinned"]:
                        show_strategy_mode()
            ...
            # The legacy strategy selector above is intentionally destroyed
            # after preferences have been migrated.  Keeping the construction
            # code for one release preserves rollback compatibility while the
            # actual distributed panel exposes only the companion workflow.
            config_frame.destroy()
            config_frame = tk.Frame(content, bg=_BG)
            companion_config = tk.Frame(config_frame, bg=_BG, padx=72, pady=20)
            companion_config.pack(fill="both", expand=True)
            ...
            capture_status_banner.pack(fill="x", pady=(0, 12))

            start_companion = tk.Label(
                companion_config,
                text="开始观察牌局",
                bg=_BTN_PRIMARY,
                fg="#ffffff",
            ...
            start_companion.pack(fill="x", pady=(0, 16))

            def _start_companion(_event) -> None:
                session_state["started"] = True
                session_state["settings_pinned"] = False
                try:
                    if self._on_start is not None:
                        ...
                        layout["companion_last_error"] = "当前只是面板预览，未连接 N.E.K.O 捕获后端。"
                except Exception as exc:
                    self.last_error = f"button click failed: {exc}"
                    layout["companion_last_error"] = self.last_error
                _refresh_companion_runtime_status()
                if bool(layout["live_running"]):
                    show_strategy_mode()

            start_companion.bind("<Enter>", lambda _event: start_companion.config(bg=_BTN_HOVER))
            start_companion.bind("<Leave>", lambda _event: start_companion.config(bg=_BTN_PRIMARY))
            start_companion.bind("<Button-1>", _start_companion)

            companion_row = tk.Frame(companion_config, bg=_CARD, padx=14, pady=10)
            companion_row.pack(fill="x", pady=(0, 10))
            companion_copy = tk.Frame(companion_row, bg=_CARD)
            companion_copy.pack(side="left", fill="both", expand=True)
            companion_status_title = tk.Label(
                companion_copy,
                text="功能已开启 · 等待开始捕获",
            ...
            companion_toggle.pack(side="right", padx=(8, 0))

            def _refresh_companion_toggle() -> None:
                enabled = bool(layout["neko_companion_enabled"])
                companion_toggle.config(
                    text="已开启  ✓" if enabled else "已静音",
                    bg=_BTN_PRIMARY if enabled else _CARD,
                    fg="#ffffff" if enabled else _TEXT,
                )
            ...
            def _refresh_companion_runtime_status() -> None:
                ...
                companion_status_detail.config(text=detail)
                capture_status_banner.config(
                    text=detail if warning else title,
                    bg=_ACTION_DEFENSE[1] if warning else _ACTION_PUSH[1],
                )

            def _save_companion_preferences() -> None:
                layout["guidance_mode"] = "companion"
                layout["panel_mode"] = "compact"
                layout["strategy_preset"] = "simple"
                _save_prefs(
                    self._prefs_path,
                    layout["width"],
                    layout["height"],
            ...
            def _toggle_companion_only(_event) -> None:
                layout["neko_companion_enabled"] = not bool(layout["neko_companion_enabled"])
                if not bool(layout["neko_companion_enabled"]):
                    layout["absurd_banter_enabled"] = False
                try:
                    _save_companion_preferences()
                except Exception as exc:
                    self.last_error = f"preference change failed: {exc}"
                _refresh_companion_toggle()
                _refresh_companion_runtime_status()

            companion_toggle.bind("<Button-1>", _toggle_companion_only)
            _refresh_companion_toggle()
            _refresh_companion_runtime_status()
            ...
            runtime_status_bar = tk.Frame(strategy_frame, bg=_SUCCESS, padx=16, pady=7)
            runtime_status_bar.pack(side="top", fill="x", padx=10, pady=(8, 4))
            runtime_status_title = tk.Label(
                runtime_status_bar,
                text="● 正在捕获 · 等待牌局事件",
                bg=_SUCCESS,
                fg="#ffffff",
            ...
            runtime_status_title.pack(fill="x")
            runtime_status_detail = tk.Label(
                runtime_status_bar,
                text="出现牌局变化后会提交吐槽或观察。",
                bg=_SUCCESS,
                fg="#ffffff",
                font=(_FONT, 10),
                anchor="w",
            )
            runtime_status_detail.pack(fill="x")

            def _refresh_running_status_bar() -> None:
                title, detail, warning = _companion_runtime_copy(
                    enabled=bool(layout["neko_companion_enabled"]),
                    live_running=bool(layout["live_running"]),
                    delivery_stage=str(layout["companion_delivery_stage"]),
                ...
                bg = _WARNING if warning or not bool(layout["neko_companion_enabled"]) else _SUCCESS
                runtime_status_bar.config(bg=bg)
                runtime_status_title.config(text=f"● {title}", bg=bg)
                runtime_status_detail.config(text=detail, bg=bg)
            ...
            local_panel, local_badge, local_label = build_panel(strategy_frame, "吐槽伙伴", _BADGE_LOCAL_BG, _BADGE_LOCAL_FG)
            ...
            def _style_action_label(badge_label: tk.Label, text: str) -> None:
                for prefix, cfg in [
                    ...
                    if text.startswith(prefix):
                        ...
                        return
                badge_label.config(text="吐槽伙伴", bg=_BADGE_LOCAL_BG, fg=_BADGE_LOCAL_FG)
            ...
            def apply_payload(
                ...
                _refresh_companion_runtime_status()
                _refresh_running_status_bar()
                _update_detail_text_widget()
                _update_detail_image()
                _render_full_strategy_card(strategy_card)
                apply_text(text, strategy_card_text)
                if _should_auto_show_running_panel(
                    started=bool(session_state["started"]),
                    live_running=bool(layout["live_running"]),
                    settings_pinned=bool(session_state["settings_pinned"]),
                    config_visible=bool(config_frame.winfo_ismapped()),
                ):
                    show_strategy_mode()
                if strategy_frame.winfo_ismapped():

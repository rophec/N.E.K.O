...
def build_public_payload(payload: dict[str, Any], *, mode: str) -> dict[str, Any]:
    ...
    Companion mode remains observation-only. Strategy mode exposes the
    engine's already-computed primary discard and three candidate comparisons
    so the dashboard and native overlay cannot disagree about the next play.
    """
    ...
    public.update(
        {
            ...
            "defense_risk_budget": float(state.get("defense_risk_budget") or 0.0),
            "target_shapes": list(presentation.get("evidence") or []),
            "caution_points": list(presentation.get("risk_sources") or []),
...
def _build_presentation(
    ...
    if mode == "companion":
        ...
        # Risk diagnostics and confidence fallbacks belong to the detailed
        # analysis surface. Feeding them to the companion on every frame made
        # the character repeatedly paraphrase internal fallbacks as
        # "not enough information" even when the recognized table facts were
        # otherwise usable.
        detail_parts = [strategy_direction, shape_summary, strategy_reason]
    else:
        if source_decision_type == "win_window":

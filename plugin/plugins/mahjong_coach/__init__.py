...
from .models import LiveSessionState, MahjongCoachConfig, _clean_string_list, _valid_play_style, _valid_river_tracking_mode, _valid_tile_recognition_mode
...
class MahjongCoachPlugin(NekoPluginBase):
    ...
    async def mahjong_coach_frame_preview(self, **_):
        if self._engine is None:
            return Err(SdkError("mahjong coach is not initialized"))
        ...
        frames_dir = self.data_path("live_frames").resolve()
        if not frame_path.is_relative_to(frames_dir) or not frame_path.is_file():
    ...
    async def mahjong_coach_table_region_preview(self, **_):
        if self._engine is None:
            return Err(SdkError("mahjong coach is not initialized"))
        raw_path = str(self._live_state.last_frame_path or "").strip()
        if not raw_path:
    ...
    async def mahjong_coach_settlement_preview(self, **_):
        if self._engine is None:
            return Err(SdkError("mahjong coach is not initialized"))
        raw_path = str(self._live_state.last_frame_path or "").strip()
        if not raw_path:
        ...
        try:
            while self._live_stop_event is not None and not self._live_stop_event.is_set():
                ...
                try:
                    ...
                    gap_state = (
                        "none"
                        if round_transition or decision.decision_type in settlement_decisions
                    ...
                    self._last_decision = decision.to_dict()
                    if not round_transition and decision.decision_type not in settlement_decisions:
                        self._observe_live_hand_change()
                    if (
                        gap_state not in {"view_obstructed", "verifying_new_round"}
                        and decision.decision_type not in settlement_decisions
                    ...
                    if decision.decision_type in settlement_decisions:
                    ...
                    if (
                        ...
                        and decision.decision_type not in settlement_decisions
                        and gap_state not in {"view_obstructed", "verifying_new_round"}
                    ):
    ...
    def _log_live_timing(
        ...
        *,
        ...
        self.logger.info(
            "mahjong coach river tracking mode={} reason={} ok={} tiles={} new={} class_corrected={} owner_migrated={} retracted={} preserved_missing={} pending={} full_rescan={} elapsed_ms={}",
            entry.get("river_mode"),
            entry.get("river_reason"),
            entry.get("river_ok"),
            entry.get("river_tile_count"),
            entry.get("river_new_discard_count"),
            entry.get("river_class_corrected_count"),
            entry.get("river_owner_migrated_count"),
            entry.get("river_retracted_count"),
            entry.get("river_preserved_missing_count"),
...
def _build_table_region_preview_payload(
    ...
    *,
    ...
    return {
        ...
        "detection_count": len(raw_detections or []),
...
def _mahjong_timing_from_decision(decision: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "river_class_corrected_count": int(river_hints.get("river_class_corrected_count") or 0),
        "river_owner_migrated_count": int(river_hints.get("river_owner_migrated_count") or 0),
        "river_retracted_count": int(river_hints.get("river_retracted_count") or 0),
        "river_preserved_missing_count": int(river_hints.get("river_preserved_missing_count") or 0),
        "river_correction_events": list(river_hints.get("river_correction_events") or []),
        "river_owner_correction_events": list(river_hints.get("river_owner_correction_events") or []),
        "river_retraction_events": list(river_hints.get("river_retraction_events") or []),

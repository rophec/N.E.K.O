...
from plugin.plugins.mahjong_coach.overlay import (
    ...
    _overlay_panel_text,
    _should_auto_show_running_panel,
...
def test_running_panel_auto_switch_respects_manual_settings_pin() -> None:
    assert _should_auto_show_running_panel(
        started=True,
        live_running=True,
        settings_pinned=False,
        config_visible=True,
    )
    assert not _should_auto_show_running_panel(
        started=True,
        live_running=True,
...
def test_companion_runtime_copy_distinguishes_switch_from_capture_and_delivery() -> None:
    ...
    assert "开始观察牌局" in detail
    assert warning is True

    title, detail, warning = _companion_runtime_copy(
        enabled=True,
        live_running=False,
        last_error="当前只是面板预览，未连接 N.E.K.O 捕获后端。",
    )
    assert title == "未能开始观察"
    assert "未连接 N.E.K.O" in detail
    assert warning is True
    ...
    assert title == "猫娘已经在看你的屏幕啦"
    assert detail == "接下来会陪你打牌助阵哦！"
...
def test_overlay_preferences_migrate_legacy_strategy_panel_to_companion(tmp_path: Path) -> None:
    prefs_path = tmp_path / "overlay.json"
...
def test_recommendation_preference_write_preserves_disabled_companion(tmp_path: Path) -> None:
    ...
    assert prefs["guidance_mode"] == "companion"
...
@pytest.mark.skipif(os.name != "nt", reason="Win32 overlay fallback is Windows-only")
...
def test_external_overlay_exposes_an_obvious_style_switch_action() -> None:
    ...
    assert _SWITCH_STYLE_LABEL == "伙伴设置"
    assert "不提供打牌建议" in _CONFIG_STYLE_HINT
    assert _win32_topbar_action("strategy", width - 80, 20, width) == "minimize"
    assert _win32_topbar_action("strategy", width - 230, 20, width) == ""
    assert _win32_topbar_action("strategy", width - 10, 20, width) == "close"
    assert _win32_topbar_action("config", width - 80, 20, width) == "minimize"


def test_win32_close_button_paint_and_hit_boxes_share_client_coordinates() -> None:
    client_width = 903
    close_rect, minimize_rect, switch_rect = _win32_topbar_rects(client_width)

    assert close_rect == (859, 0, 903, 60)
    assert minimize_rect == (815, 0, 859, 60)
    assert switch_rect == (675, 0, 815, 60)
    assert _win32_topbar_action("strategy", 814, 20, client_width) == "switch_style"
    assert _win32_topbar_action("strategy", 815, 20, client_width) == "minimize"
...
def test_minimize_keeps_overlay_thread_and_live_owner_running(tmp_path: Path) -> None:
    calls: list[str] = []
    controller = CoachOverlayController(
        prefs_path=tmp_path / "overlay.json",
        on_stop=lambda: calls.append("host_stop"),
    ...
    assert controller.window_visible is False
    assert controller._stop_requested.is_set() is False
    assert controller._queue.get_nowait() == {"cmd": "minimize"}
    assert calls == []

    controller.restore()
    assert controller._queue.get_nowait() == {"cmd": "restore"}
    assert calls == []
...
def test_win32_config_cards_have_clear_non_overlapping_hit_regions() -> None:
    ...
    assert _win32_config_action(700, 180, width) == ("start", "riichi")
    assert _win32_config_action(600, 350, width) == ("companion_toggle", "")
    assert _win32_config_action(600, 490, width) == ("", "")
    assert _win32_config_action(300, 350, width) == ("", "")
    assert _win32_config_action(300, 490, width) == ("", "")
    assert _win32_config_action(600, 600, width) == ("", "")

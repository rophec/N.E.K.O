...
def test_action_button_template_scan_detects_pon(tmp_path: Path) -> None:
    ...
    assert meta["templates"]["matches"][0]["button_type"] == "pon"
    match_left, match_top = meta["templates"]["matches"][0]["match_box"][:2]
    assert abs(match_left - 800) <= 2
    assert abs(match_top - 660) <= 2


def test_action_button_template_scan_detects_ron_without_call_buttons(tmp_path: Path) -> None:
    template_path = (
        Path(__file__).resolve().parents[1]
        / "perception"
        / "templates"
    ...
    template = Image.open(template_path).convert("RGB")
    image = Image.new("RGB", (1920, 1080), (20, 30, 40))
    image.paste(template, (820, 660))
    frame_path = tmp_path / "ron_frame.png"
    image.save(frame_path)

    buttons, meta = detect_action_buttons_fast(frame_path)

    assert buttons == ["ron"]
    assert meta["templates"]["matches"][0]["button_type"] == "ron"
...
def test_action_detector_rejects_conflicting_button_sets(tmp_path: Path, monkeypatch) -> None:
    ...
    ImageDraw.Draw(image).rectangle((700, 650, 1000, 760), fill=(245, 245, 235))
...
def test_action_detector_does_not_treat_table_tiles_as_win_buttons(tmp_path: Path) -> None:
    image = Image.new("RGB", (1920, 1080), (29, 72, 126))
    draw = ImageDraw.Draw(image)
    for left in range(650, 1250, 70):
        draw.rounded_rectangle(
            (left, 610, left + 55, 690),
            radius=4,
            fill=(238, 238, 226),
            outline=(210, 140, 35),
            width=4,
        )
    frame_path = tmp_path / "table_tiles_without_buttons.png"
    image.save(frame_path)

    buttons, meta = detect_action_buttons_fast(frame_path)

    assert buttons == []
    ron_match = next(item for item in meta["templates"]["matches"] if item["button_type"] == "ron")
    assert ron_match["accepted"] is False

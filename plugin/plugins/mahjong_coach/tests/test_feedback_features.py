...
def test_guidance_and_inference_defaults_migrate_without_actionable_output() -> None:
    ...
    assert MahjongCoachConfig.from_payload(
        {"perception": {"inference_provider": ""}}
    ).inference_provider == "speed"
    assert MahjongCoachConfig.from_payload(
        {"perception": {"inference_provider": "future_gpu_alias"}}
    ).inference_provider == "speed"
    assert MahjongCoachConfig.from_payload(
        {"perception": {"inference_provider": "cpu"}}
...
def test_public_modes_keep_companion_observational_and_make_strategy_actionable() -> None:
    ...
    assert "边防守边推进" in companion["presentation"]["strategy_direction"]
...
def test_compact_overlay_shows_persisted_closest_shape_without_listing_hand() -> None:
    ...
    assert public["presentation"]["shape_summary"] == "最近牌型：七对子｜1向听｜有效进张约9张"
    compact = overlay_text_from_payload(public)
    assert "最近牌型：七对子｜1向听｜有效进张约9张" in compact
...
def test_neko_companion_does_not_speak_internal_insufficiency_fallbacks() -> None:
    ...
    # No-riichi normal play must not be mislabeled as missing river evidence.
    assert any("当前无人立直" in item for item in public["presentation"]["risk_sources"])
    assert any("尚无稳定置信度" in item for item in public["presentation"]["uncertainty"])
...
def test_neko_companion_keeps_shape_distance_and_zero_melds_out_of_speech() -> None:
    ...
    # Detailed analysis retains these facts; spoken context deliberately does not.
    assert any(
        "大三元" in item and "距离约8张" in item
        for item in public["presentation"]["evidence"]
    )
    assert any("自己0组副露" in item for item in public["presentation"]["evidence"])
...
async def test_neko_companion_push_is_hidden_deduplicated_and_non_prescriptive() -> None:
    ...
    assert message["coalesce_key"] == "mahjong_coach:mahjong_companion"
...
@pytest.mark.asyncio
async def test_neko_companion_ambient_cadence_fills_quiet_play_without_spam(monkeypatch) -> None:
    plugin = MahjongCoachPlugin.__new__(MahjongCoachPlugin)
    plugin._cfg = MahjongCoachConfig(
        live_advice_mode="companion",
        neko_companion_enabled=True,
    ...
    clock = {"now": 100.0}
    monkeypatch.setattr(mahjong_plugin_module.time, "time", lambda: clock["now"])
...
@pytest.mark.asyncio
async def test_neko_companion_critical_riichi_bypasses_ordinary_cooldown(monkeypatch) -> None:
    plugin = MahjongCoachPlugin.__new__(MahjongCoachPlugin)
    ...
    pushed: list[dict[str, object]] = []
    plugin.push_message = lambda **kwargs: pushed.append(kwargs)
    monkeypatch.setattr(mahjong_plugin_module.time, "time", lambda: 101.0)
    snapshot = build_public_payload(
        {
            "last_decision": {"decision_type": "defense_alert"},
            "round_state": {

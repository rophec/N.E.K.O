...
def test_dashboard_activity_temporarily_raises_preview_cadence(
    monkeypatch: pytest.MonkeyPatch,
    patch_module_clock,
) -> None:
    plugin = MahjongCoachPlugin.__new__(MahjongCoachPlugin)
    ...
    patch_module_clock(monkeypatch, mahjong_plugin_module, monotonic=lambda: 100.79)
    assert plugin._should_persist_preview(decision) is False
    patch_module_clock(monkeypatch, mahjong_plugin_module, monotonic=lambda: 100.81)
    assert plugin._should_persist_preview(decision) is True

    plugin._live_preview_requested_at = 90.0
    patch_module_clock(monkeypatch, mahjong_plugin_module, monotonic=lambda: 103.99)
    assert plugin._should_persist_preview(decision) is False
    patch_module_clock(monkeypatch, mahjong_plugin_module, monotonic=lambda: 104.0)
    assert plugin._should_persist_preview(decision) is True

    patch_module_clock(monkeypatch, mahjong_plugin_module, monotonic=lambda: 100.01)

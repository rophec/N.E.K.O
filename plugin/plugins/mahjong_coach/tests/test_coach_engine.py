...
def test_riichi_defense_uses_recognized_riichi_player_river(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert decision.analysis_source == "deterministic_genbutsu"
    assert decision.perception["defense"]["rule"] == "own_discard_furiten"
    assert decision.perception["defense"]["rule_scope"] == "ron_only"
    assert decision.perception["defense"]["held_safe_against_all"] == ("2s",)
    assert decision.coach_state["last_defense"] == decision.perception["defense"]

...
def test_distributed_manifest_and_dashboard_are_companion_only() -> None:
    manifest = tomllib.loads(PLUGIN_TOML.read_text(encoding="utf-8"))
    html = (MAIN_JS.parent / "index.html").read_text(encoding="utf-8")
    source = MAIN_JS.read_text(encoding="utf-8")

    assert "live_advice_mode" not in manifest["decision"]
    assert "strategy_preset" not in manifest["decision"]
    assert 'id="guidanceModeInput" type="checkbox"' in html
    assert 'class="toggle-field core-toggle-field companion-retired" hidden' in html
    assert 'class="control-field companion-retired" hidden' in html
    assert "function guidanceModeValue() {\n  return 'companion';\n}" in source
    assert "strategy_preset: 'simple'" in source
    assert "live_advice_mode: 'companion'" in source
...
def test_static_ui_collapses_infrequent_runtime_controls() -> None:
    ...
    runtime_controls = html.split('<details class="advanced-control-band runtime-settings-band companion-retired" hidden>', 1)[1].split(
        "</details>", 1
    )[0]

    assert 'id="keywordsInput"' in core_controls
    assert 'id="guidanceModeInput"' in core_controls
    assert 'id="overlayInput"' not in core_controls
...
def test_companion_dashboard_hides_legacy_controls_and_shows_delivery_state() -> None:
    html = (MAIN_JS.parent / "index.html").read_text(encoding="utf-8")
    source = MAIN_JS.read_text(encoding="utf-8")

    assert 'id="startYoloLiveBtn" class="yolo-live-button companion-retired" hidden' in html
    assert 'id="resetBtn" class="secondary-button companion-retired" hidden' in html
    assert 'id="companionStatusTitle"' in html
    assert 'id="companionChannelState"' in html
    assert 'id="companionEventKind"' in html
    assert 'id="companionSubmitCount"' in html
    assert "function renderCompanionStatus(sync = {}, live = {})" in source
    assert "renderCompanionStatus(data.neko_companion_sync || {}, live);" in source
...
def test_running_dashboard_rehydrates_runtime_choices_from_backend() -> None:
    ...
    assert "absurdBanterEnabledInput" not in source
    assert "syncAbsurdBanterAvailability" not in source
    assert "absurd_banter_enabled: false" in source

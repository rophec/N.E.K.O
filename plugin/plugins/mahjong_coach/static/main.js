...
function renderEfficiency(efficiency = {}) {
  // 展示确定性牌效；“尚未看见”不等同于“牌山剩余”。
  // Show deterministic efficiency; unseen copies are not guaranteed wall copies.
  const shanten = Number(efficiency.current_shanten);
  if (!Number.isFinite(shanten)) {
    efficiencyValue.textContent = '等待手牌';
    efficiencyDetail.textContent = '尚未计算有效牌';
    return;
  }
  const shantenText = shanten < 0 ? '已和牌' : shanten === 0 ? '听牌' : `${shanten} 向听`;
  const options = Array.isArray(efficiency.discard_options) ? efficiency.discard_options : [];
  const currentTypes = Number(efficiency.current_effective_types || 0);
  const currentCount = Number(efficiency.current_effective_count || 0);
  const best = options[0];
  efficiencyValue.textContent = shantenText;
  if (best) {
    const waits = Array.isArray(best.effective_tiles) ? best.effective_tiles.join(' ') : '';
    efficiencyDetail.textContent = `打 ${best.tile}：${Number(best.effective_types || 0)} 种 ${Number(best.effective_count || 0)} 枚尚未看见${waits ? ` (${waits})` : ''}`;
  } else if (currentTypes || currentCount) {
    const waits = Array.isArray(efficiency.current_effective_tiles) ? efficiency.current_effective_tiles.join(' ') : '';
    efficiencyDetail.textContent = `${currentTypes} 种 ${currentCount} 枚尚未看见${waits ? ` (${waits})` : ''}`;
  } else {
    efficiencyDetail.textContent = '当前没有可用的有效牌结果';
  }
}
...
function renderMelds(melds = []) {
  meldTiles.replaceChildren();
  const values = Array.isArray(melds) ? melds : [];
  meldCount.textContent = `${values.length} 组`;
  if (!values.length) {
    const empty = document.createElement('span');
    empty.className = 'empty-text';
    empty.textContent = '暂无副露';
    meldTiles.appendChild(empty);
    return;
  }
...
function renderDashboard(data = {}) {
  ...
  if (tileRecognitionModeInput && config.tile_recognition_mode) {
    tileRecognitionModeInput.value = config.tile_recognition_mode;
  }
  // 防守警报必须覆盖旧的进攻计划，避免正确的现物建议被缓存内容遮住。
  // A defense alert must override cached attack plans so genbutsu advice stays visible.
  const localPlan = decision.decision_type === 'defense_alert'
    ? decision.suggestion
    : state.local_direction || state.local_plan || state.current_plan || state.opening_plan || decision.suggestion;
  ...
  const planSource = String(decision.analysis_source || state.plan_source || 'heuristic');
  analysisSource.textContent = planSource.includes('deterministic_genbutsu')
    ? '现物规则'
    : planSource.includes('deterministic_efficiency')
      ? '精确牌效 + 启发式路线'
      : '启发式策略';
  ...
  renderEfficiency(state.last_efficiency || {});

...
const absurdBanterEnabledInput = document.getElementById('absurdBanterEnabledInput');
...
function guidanceModeValue() {
  return guidanceModeInput?.checked ? 'strategy' : 'companion';
}

function setGuidanceMode(value) {
  if (guidanceModeInput) {
    guidanceModeInput.checked = value === 'strategy';
  }
}

function syncAbsurdBanterAvailability() {
  if (!absurdBanterEnabledInput) return;
  const available = Boolean(nekoCompanionEnabledInput?.checked);
  absurdBanterEnabledInput.disabled = !available;
  if (!available) absurdBanterEnabledInput.checked = false;
}
...
function renderPreferences(data = {}, force = false) {
  ...
  if (!preferencesHydrated || force) {
    ...
    if (absurdBanterEnabledInput && data.config?.absurd_banter_enabled !== undefined) {
      absurdBanterEnabledInput.checked = Boolean(
        data.config.neko_companion_enabled && data.config.absurd_banter_enabled
      );
    }
    syncAbsurdBanterAvailability();
    if (target.title) {
...
function requestYoloWarmup() {
  ...
  yoloWarmupPromise = callPlugin('mahjong_coach_warmup_yolo26', {
    ...
    if (result.status === 'ready' && result.accelerated === true) {
      yoloWarmupLabel = `GPU 已就绪 ${Number(result.elapsed_ms || 0).toFixed(0)} ms`;
    } else if (result.status === 'fallback' || result.accelerated === false) {
      const reason = compact(result.fallback_reason || result.reason, 'GPU provider 不可用');
      yoloWarmupLabel = `已回退 CPU · ${reason}`;
    } else {
...
function syncYoloWarmupState(state = {}) {
  if (state.status === 'warming') {
    yoloWarmupLabel = 'GPU 预热中';
  } else if (state.status === 'ready' && state.accelerated !== false) {
    yoloWarmupLabel = `GPU 已就绪 ${Number(state.elapsed_ms || 0).toFixed(0)} ms`;
  } else if (state.status === 'fallback' || state.accelerated === false) {
    const reason = compact(state.fallback_reason || state.reason, 'GPU provider 不可用');
    yoloWarmupLabel = `已回退 CPU · ${reason}`;
...
async function analyzeFrame() {
  ...
  const data = await callPlugin('mahjong_coach_analyze_frame', {
    ...
    live_advice_mode: guidanceModeValue(),
...
async function startLive() {
  ...
  const data = await callPlugin('mahjong_coach_start_live', {
    ...
    absurd_banter_enabled: Boolean(
      nekoCompanionEnabledInput?.checked && absurdBanterEnabledInput?.checked
    ),
    inference_provider: inferenceProviderInput ? inferenceProviderInput.value : 'speed',
...
async function startYoloLive() {
  // 中文：快捷入口固定启用 YOLO26、实时牌河和 GPU 优先模式。
  // English: The quick entry enables YOLO26, live river tracking, and GPU-first inference.
  setRecognitionMode('yolo26');
  riverTrackingModeInput.value = 'live';
  if (inferenceProviderInput) inferenceProviderInput.value = 'speed';
  renderRuntimeConfigSummary();
  await requestYoloWarmup().catch(() => {});
  await startLive();
}
...
nekoCompanionEnabledInput?.addEventListener('change', syncAbsurdBanterAvailability);

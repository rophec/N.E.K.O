...
function renderRiverCollection(container, countNode, piles = {}, emptyText = '暂无牌河') {
  ...
  entries.forEach(([player, items]) => {
    ...
    (Array.isArray(items) ? items : []).forEach((item) => {
      ...
      node.className = `tile tile-small tile-${tile.slice(-1)}`;
      node.textContent = tile;
      // 中文：牌河历史仍显示被鸣走的牌；待撤回记录则明确标出复核进度。
      // English: Keep called-away history visible and expose pending retraction evidence.
      const missingConfirmations = Number(item?.missing_confirmations || 0);
      const notes = [];
      if (missingConfirmations > 0) {
        node.classList.add('tile-river-pending');
        notes.push(`连续 ${missingConfirmations} 次复核未见，等待纠错确认`);
      }
      if (item?.currently_visible === false && item?.preserve_history) {
        node.classList.add('tile-river-history');
        notes.push('当前桌面不可见，但作为已鸣走的弃牌历史保留');
      }
      if (item?.corrected_from) {
        notes.push(`牌名已从 ${item.corrected_from} 纠正为 ${tile}`);
      }
      if (item?.corrected_player_from) {
        notes.push(`玩家归属已从 ${item.corrected_player_from} 迁移到 ${player}`);
      }
      if (notes.length) {
...
function buildPipelineSteps(data = {}) {
  ...
  const riverHints = river.analysis_hints || {};
  const classCorrected = Number(riverHints.river_class_corrected_count || 0);
  const ownerMigrated = Number(riverHints.river_owner_migrated_count || 0);
  const retracted = Number(riverHints.river_retracted_count || 0);
  const pendingCorrections = Number(riverHints.river_pending_corrections || 0);
  if (classCorrected + ownerMigrated + retracted > 0) {
    riverDetail = `本帧纠错：牌名 ${classCorrected}，归属 ${ownerMigrated}，撤回 ${retracted}`;
  } else if (pendingCorrections > 0) {
    riverDetail = `纠错确认中：${pendingCorrections} 项`;
  }
